from PySide6.QtWidgets import QWidget, QLabel, QPushButton, QMessageBox, QDialog, QVBoxLayout, QLineEdit
from PySide6.QtGui import QPixmap, QPainter, QColor, Qt, QImage, QPainterPath, QIcon
from userAccount.ui_userAccountStudent import Ui_UserAccountStudent
from DB.connect_to_db import connect_to_database
from PySide6.QtCore import QTimer


class UserAccountStudent(QWidget):
    def __init__(self, user_id = None, parent=None):
        super().__init__(parent)
        self.ui = Ui_UserAccountStudent()
...
#        self.ui.choosePhoto.clicked.connect(self.choose_photo)
    ...
    def set_user_data(cursor, self):
        ...
        # University info
        cursor.execute("""
        SELECT classes.name AS class_name, institutes.name AS institute_name
        FROM students
        JOIN classes ON students.class_id = classes.id
        JOIN institutes ON classes.institute_id = institutes.id
        WHERE students.user_id = ?
        """, (self.user_id,))

        group, institute_name  = cursor.fetchone()

        self.ui.group.setText(group)
...
#            cursor.execute('UPDATE users SET way_to_photo = ? WHERE id = ?', (file_name, self.user_id))
    ...
    def edit_labels(cursor, self):
        dialog = EditLabelsDialog(self.ui.firstName.text(), self.ui.lastName.text(), self.ui.patronymic.text(), self.ui.description.text(), self.ui.phone.text(), self.ui.email.text())
        result = dialog.exec_()
...
class EditLabelsDialog(QDialog):
    def __init__(self, first_name="", last_name="", patronymic="", description="", phone="", email="", parent=None):
        super().__init__(parent)
        ...
        self.labelFN = QLabel('Прізвище', self)
        self.labelSN = QLabel("Ім'я", self)
